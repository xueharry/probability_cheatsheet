...
\subsection{Cauchy}A student-t with 1 degree of freedom is called a Cauchy. The reciprocal of a Cauchy r.v. is still Cauchy.
\subsection{Inequalities}
\begin{enumerate}
\item$P(X+Y)>4 \geq P(X > 2, Y > 2)$ since the event on the right is a subset of the event on the left.\\
\item$E(X^2e^X)\geq E(X^2)E(e^X)$ since $X^2$ and $e^X$ are positively correlated.\\
\item $E(\frac{X}{Y}+\frac{Y}{X} \geq 2)$ since $E(X)E(\frac{1}{Y})\geq\frac{EX}{EY}=1)$.\\
\item For i.i.d. continuous random variables, $P(X<Y)=\frac{1}{2}$ (since all orderings are equally likely).\\
\item $P(Z \geq \frac{1}{X^4+Y^4+7})<1$ since $Z$ may be negative and $\frac{1}{X^4+Y^4+7}$ is positive.\\
\item For Standard Normal r.v.s, applying the \textbf{68-95-99.8} rule may be helpful.\\
\item By the symmetry of the Normal, $E(e^{-X})=E(e^X)$ for a standard normal r.v.
\end{enumerate}
\subsection{Monty Hall}
Assume contestant picks door 1, assume switching strategy, condition on location of car
\subsection{Store with random number of customers}
...
Starting with a proposal chain, we generate a new chain. Let $X_0$ be any starting state and generate a chain $X_0$, $X_1$,... such that each "move" is as follows. If the chain is currently at state $i$, then:
\begin{enumerate}
\item Generate a proposal state $j$ according to the original transition probability\\
\item Accept the proposal with probability $min(1, \frac{d_i}{d_j})$. If the proposal is accepted, go to $j$; otherwise, stay at $i$.
\end{enumerate}
\subsection{Tips}
\begin{enumerate}
\item When in doubt, condition on what you wish to know.
\item When a calculation looks nasty, try to think of a story.
\item \textbf{Indicators}- consider pairs
\item \textbf{Exponential}- memoryless property
\item \textbf{Variance}- try the sum of variances minus covariances definition
\item \textbf{Covariance}- Check for story of the mutinomial. Otherwise apply the definition.
\item Consider \textbf{Adam's or Eve's Law} for expectation or variance
\end{enumerate}
\subsection{Common Stories} Birthday Problem, Chicken and Egg, Fisher Exact Test, Bank-Post Office, Binomial, Hypergeometric, Poisson Paradigm, Universality of the Uniform, Bayes' Billiards, Adam and Eve's Law, Random Walk on Undirected Network, Metropolis Algorithm
